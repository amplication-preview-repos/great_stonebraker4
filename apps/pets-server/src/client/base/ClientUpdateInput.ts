/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { BookingUpdateManyWithoutClientsInput } from "./BookingUpdateManyWithoutClientsInput";
import { Type } from "class-transformer";
import { ReviewUpdateManyWithoutClientsInput } from "./ReviewUpdateManyWithoutClientsInput";
import { DogUpdateManyWithoutClientsInput } from "./DogUpdateManyWithoutClientsInput";

@InputType()
class ClientUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  phoneNumber?: string | null;

  @ApiProperty({
    required: false,
    type: () => BookingUpdateManyWithoutClientsInput,
  })
  @ValidateNested()
  @Type(() => BookingUpdateManyWithoutClientsInput)
  @IsOptional()
  @Field(() => BookingUpdateManyWithoutClientsInput, {
    nullable: true,
  })
  bookings?: BookingUpdateManyWithoutClientsInput;

  @ApiProperty({
    required: false,
    type: () => ReviewUpdateManyWithoutClientsInput,
  })
  @ValidateNested()
  @Type(() => ReviewUpdateManyWithoutClientsInput)
  @IsOptional()
  @Field(() => ReviewUpdateManyWithoutClientsInput, {
    nullable: true,
  })
  reviews?: ReviewUpdateManyWithoutClientsInput;

  @ApiProperty({
    required: false,
    type: () => DogUpdateManyWithoutClientsInput,
  })
  @ValidateNested()
  @Type(() => DogUpdateManyWithoutClientsInput)
  @IsOptional()
  @Field(() => DogUpdateManyWithoutClientsInput, {
    nullable: true,
  })
  dogs?: DogUpdateManyWithoutClientsInput;
}

export { ClientUpdateInput as ClientUpdateInput };
